...
public class FluidSim3D : MonoBehaviour
{
	...
	public Texture3D border;
	public Texture3D flow;
	...
	void Start()
	{
		...
		tex = Instantiate(GetComponent<Renderer>().material.mainTexture) as Texture3D;
		...
		for (int i = 0; i < size; i++) {
			dens_prev[i] = u_prev[i] = v_prev[i] = w_prev[i] = dens[i] = u[i] = v[i] = w[i] = 0;
//			int y = i / rowSize;
//			int x = i % rowSize;
//			int z = 0;
//			bndX[i] = border.GetPixel(x, y, z).grayscale - border.GetPixel(x+1, y, z).grayscale;
//			bndY[i] = border.GetPixel(x, y, z).grayscale - border.GetPixel(x, y+1, z).grayscale;
//			bndZ[i] = border.GetPixel(x, y, z).grayscale - border.GetPixel(x, y, z+1).grayscale;
//			velX[i] = (flow.GetPixel(x, y, z).grayscale - flow.GetPixel(x+1, y, z).grayscale) * 0.1f;
//			velY[i] = (flow.GetPixel(x, y, z).grayscale - flow.GetPixel(x, y+1, z).grayscale) * 0.1f;
//			velZ[i] = (flow.GetPixel(x, y, z).grayscale - flow.GetPixel(x, y, z+1).grayscale) * 0.1f;
		}
	}
